...
\- sample inputs: \`x \~ N(0, I\_d)\`  
\- labels: \`y \= w^T x\`  
\- in a context of \`n-1\` labeled examples plus a final query \`x\_n\`, the transformer predicts \`ŷ\_n\`.  
...
\- \`fig\_sphere.png\` — schematic figure used in documentation/plots  